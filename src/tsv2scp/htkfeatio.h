...
public:
	...
	size_t  getCurrentFrames() {return curframe; }
	wstring getPhysicalPath() {return physicalpath;}
...
public:
    ...
    protected:
        ...
        string logicalpath; // virtual path that this file should be understood to belong to
    ...
    public:
		...
		void setAsArchivePath(wstring lPath, wstring pPath, size_t st, size_t en)
		{
			logicalpath = msra::strfun::utf8(lPath);
			s = st; 
			e = en; 
			isarchive = true;
			auto iter = archivePathStringMap.find(pPath);
			if (iter == archivePathStringMap.end())
			{
				archivePathIdx = (unsigned int)archivePathStringMap.size();
				archivePathStringMap[pPath] = archivePathIdx;
				archivePathStringVector.push_back(pPath);
			}
			else
			{
				archivePathIdx = iter->second; 
        ...
        wstring physicallocation2() const
        {
            return msra::strfun::wstrprintf(L"%ls[%d,%d]", physicallocation().c_str(), s, e);
        }
...
public:
	...
	htkmlfentry(unsigned int st, unsigned int len, msra::dbn::CLASSIDTYPE sid, msra::dbn::HMMIDTYPE hid)
	{
		firstframe = st; 
		numframes = len; 
		classid = sid; 
...
class htkmlfreader : public map<wstring, vector<ENTRY>> // [key][i] the data
{
	...
	vector<string>					   stateid2name; // for index => state
...
public:
    ...
    void readstatelist(const wstring& stateListPath = L"")
    {
        if (stateListPath != L"")
        {
            ...
            for (index = 0; index < lines.size(); index++)
            {
				...
				stateid2name.push_back(lines[index]);
	...
	string getstatename(size_t id){return stateid2name[id];}
    ...
    vector<string>  getStateName() const {
        return stateid2name; 
    }

	size_t getNumLabeledFrames(wstring key)
	{
		if (this->find(key) == this->end())
		{
			return 0;
		}
		vector<ENTRY>  v = this->operator[](key);
		size_t nframe = 0; 
		for (auto e : v)
		{
			nframe += e.numframes; 
